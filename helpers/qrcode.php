...
final class QrEncoder {
    ...
    /**
     * Pick smallest supported version (1-10 in this table) fitting $len bytes in byte mode at given level.
    ...
    /**
     * Encode $data (byte mode only) into a boolean matrix. Returns null if data too large (>version 10 capacity here).
     */
    public static function encode(string $data, string $level = 'M'): ?array {
        ...
        // Remainder bits per version (for versions 2-6: 7 bits; version 1: 0; we only go to v10 => 7 bits for v2-6, 0 for v7-13 per spec... version 1:0, 2-6:7, 7-13:0, 14-20:3...)
    ...
    private static function buildMatrix(int $version, string $level, array $dataBits): array {
        ...
        // Version info block (version 7+) - not needed since we cap at version 10... wait 7-10 needs version info!
        if ($version >= 7) {
    ...
    private static function alignmentCoords(int $version): array {
        // Table for versions 1-10 (sufficient for our supported range)
        $table = [
            1 => [],
            2 => [6, 18],
            3 => [6, 22],
...
final class PngWriter {
    public static function fromMatrix(array $matrix, int $moduleSize = 8, int $quietZone = 4): string {
        $n = count($matrix);
...
class QrGenerator {
    ...
    public static function generatePng(string $data): ?string {
        ...
        return PngWriter::fromMatrix($matrix, 8, 4);
    }
