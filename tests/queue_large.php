...
$uncapped=[];
for ($i=0; $i<12; $i++) $uncapped[]=BatchQueue::message(99,42,'uncapped','uncapped'.$i);
$processed=BatchQueue::run(2);
if ($processed < 12) throw new RuntimeException('Worker still has a fixed ten-step limit');
foreach ($uncapped as $item) if (BatchQueue::get($item['id'])['status']!=='completed') throw new RuntimeException('Time-budget worker left an eligible message pending');
