...
Storage::init();
function tg_send_message($chat,$text,$kb=null) { global $argv; file_put_contents($argv[2].'/sends', "send\n", FILE_APPEND | LOCK_EX); if ($argv[3]==='slow-send') { file_put_contents($argv[2].'/entered-send','1'); sleep(30); } return ['ok'=>true]; }
...
if ($argv[3]==='enqueue-outbox') { echo BatchQueue::message(42,42,'fixture message','killed-message')['id']; exit; }
...
with tempfile.TemporaryDirectory(prefix='holderbot-outbox-process-') as tmp:
    subprocess.run(cmd(tmp,'init'),check=True)
    job=subprocess.check_output(cmd(tmp,'enqueue-outbox'),text=True)
    worker=subprocess.Popen(cmd(tmp,'slow-send'),stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
    try:
        for _ in range(100):
            if (Path(tmp)/'entered-send').exists(): break
            if worker.poll() is not None: raise AssertionError(worker.communicate())
            time.sleep(.02)
        else: raise AssertionError('Send did not start')
    finally:
        worker.kill(); worker.communicate(timeout=5)
    for _ in range(3): subprocess.run(cmd(tmp,'normal'),check=True,stdout=subprocess.DEVNULL)
    state=json.loads((Path(tmp)/'queue'/job/'job.json').read_text())
    assert state['status']=='completed' and state['unconfirmed']==1,state
    assert len((Path(tmp)/'sends').read_text().splitlines())==1,'Interrupted message was sent twice'
    print('PASS: SIGKILL during Telegram delivery recovered without automatic resend')
