...
class PanelManager {
    ...
    public static function createAdmin($server,$username,$password,$sudo=false): ?array { self::$calls[]=['create_admin',$username,$sudo];self::$admins[]=$username;return ['username'=>$username,'is_sudo'=>$sudo]; }
...
check(buttons(Keyboards::serverMenu(1))===['users:1:1:all','new_usr:1','srch_usr:1','admins_list:1:1','new_adm:1','srch_adm:1','stats:1','act_menu:1','srv_cfg:1','home'],'Server menu actions do not match the requested layout');
$userListRows=Keyboards::usersList(1,[],1);check(array_column($userListRows['inline_keyboard'][1],'callback_data')===['new_usr:1','srch_usr:1'],'Users list does not place Create User and Search User together');
...
callback('admins_list:1:1');check(in_array('new_adm:1',buttons(end($events)[1][3]),true),'Administrator list omitted Create Admin');callback('new_adm:1');callback('adm_role:1:regular');check(Storage::getState(42)['step']==='create_admin_credentials','Regular administrator wizard did not reach credentials');StateHandlers::handle(['text'=>'fresh_admin securepass','chat'=>['id'=>99],'from'=>['id'=>42]],Storage::getState(42));check(end(PanelManager::$calls)===['create_admin','fresh_admin',false]&&Storage::getState(42)===null,'Regular administrator was not created safely');
callback('srch_adm:1');
